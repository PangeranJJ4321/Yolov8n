...
Validasi keandalan sistem dalam mengukur dimensi fisik lubang jalan (diameter dan kedalaman) dilakukan melalui serangkaian eksperimen lapangan menggunakan kendaraan bergerak pada kecepatan rendah hingga menengah (10--30 km/jam). Eksperimen ini bertujuan untuk menguji dua aspek fundamental: (1) stabilitas pelacakan objek dalam kondisi dinamis, dan (2) konsistensi pengukuran dimensional menggunakan parameter kalibrasi kamera mono-kamera yang telah divalidasi dengan \textit{reprojection error} sebesar 0,043 piksel.

\subsubsection{Analisis Konsistensi Estimasi Diameter}
\mbox{}\\
Hasil pengujian pada video sekuensial (\textit{pothole\_video.mp4}) menunjukkan kemampuan sistem untuk melakukan estimasi diameter secara kontinyu. Pengukuran dilakukan pada \textit{Region of Interest} (ROI) yang dinamis seiring pergerakan kendaraan mendekati objek. Data log pengukuran (Tabel 2) memperlihatkan tren yang konsisten pada objek uji utama (Track ID 2):

\begin{enumerate}
    \item \textbf{Validasi Hukum Perspektif:} Pada saat objek pertama kali terdeteksi dan dikonfirmasi oleh tracker (Frame 23), diameter terukur adalah 7,6 cm. Seiring kendaraan mendekat (Frame 40), estimasi diameter meningkat secara bertahap dan stabil menuju angka 9,8 cm. Fenomena ini mengonfirmasi berjalannya prinsip proyeksi kamera, di mana resolusi spasial per piksel meningkat pada jarak dekat, memungkinkan segmentasi yang lebih presisi pada tepi-tepi lubang (fine-grained boundary detection).
    \item \textbf{Superioritas Segmentation Mask:} Dibandingkan metode tradisional berbasis \textit{bounding box}, pendekatan \textit{instance segmentation} yang kami terapkan terbukti lebih robust terhadap noise latar belakang. Jari-jari mask yang diekstraksi mampu mengecualikan aspal sehat di sekitar lubang, sehingga mencegah over-estimasi diameter horizontal yang sering terjadi pada metode deteksi kotak sederhana.
    \item \textbf{Stabilitas Temporal:} Filter CDKF terbukti efektif dalam meredam fluktuasi pengukuran antar-frame yang disebabkan oleh getaran kamera. Variansi diameter antar-frame berhasil ditekan di bawah 0,5 cm, yang sangat krusial untuk mencegah "flickering" data pada dasbor pelaporan.
\end{enumerate}
