...
\usepackage{ragged2e} % Provides \justify
\usepackage{fancyhdr} % Custom headers
\usepackage{indentfirst} % Indent first paragraph after section
\usepackage{titlesec} % Control section spacing

% --- Spacing Configuration ---
% Add space after section/subsection and before the paragraph
...
% Note: User needs to provide 'images/logo_unhas.png'
\lhead{
    \begin{minipage}{0.6\textwidth}
        \raisebox{-0.3\height}{\includegraphics[height=0.8cm]{images/Logo-Resmi-Unhas-1.png}} 
        \hspace{0.2cm} 
        \textbf{Universitas Hasanuddin}
    \end{minipage}
}
% Right Header: Author et al.
\rhead{\textit{Jafar et al.}}
% Footer: Page number
\cfoot{\thepage}

% Redefine plain style (usually used on first page) to match
...
\title{\Huge Sistem Terintegrasi Deteksi dan Pengukuran Lubang Jalan Menggunakan YOLOv8 dan DepthAnything V2 dengan Filter Temporal}

\author{Pangeran Juhrifar Jafar}
\address{Jurusan Sistem Informasi, Universitas Hasanuddin, Makassar, Indonesia}
\ead{pangeranjuhrifar@gmail.com}
...
\begin{abstract}
\justify
\hspace{1.5em}Pemeliharaan jalan yang proaktif sangat penting untuk keselamatan dan efisiensi ekonomi, namun metode inspeksi manual masih bersifat reaktif, subjektif, dan berisiko. Makalah ini menghadirkan sistem terintegrasi untuk deteksi dan pengukuran lubang jalan secara mendekati waktu nyata (\textit{near real-time}) menggunakan perangkat \textit{edge}. Sistem memadukan \textbf{YOLOv8m-seg} untuk deteksi dan segmentasi instan, \textbf{DepthAnything V2} untuk estimasi kedalaman monokuler, serta penyaringan temporal untuk stabilitas pengukuran.

\hspace{1.5em}\textit{Pipeline} mencakup: (1) deteksi lubang dengan segmentasi instansi; (2) estimasi kedalaman dan pemulihan skala berbasis tinggi kamera untuk memperoleh ukuran metrik absolut; (3) pengukuran diameter dan kedalaman menggunakan statistik robust dan penyesuaian mask; (4) pelacakan BoT-SORT dan penyaringan \textbf{Confidence and Distance Kalman Filter (CDKF)} untuk menstabilkan estimasi; (5) pelaporan otomatis melalui REST API.

Evaluasi menunjukkan bahwa sistem mampu beroperasi secara efisien dengan kecepatan inferensi rata-rata \textbf{14--15 FPS} pada perangkat kelas konsumen. Meskipun akurasi kedalaman absolut masih dipengaruhi oleh asumsi kalibrasi, sistem menunjukkan konsistensi pengukuran relatif yang tinggi, memungkinkan klasifikasi tingkat keparahan kerusakan yang andal. Hasil ini menegaskan kelayakan sistem sebagai solusi inspeksi jalan cerdas yang hemat biaya dan cukup responsif untuk skenario pemeliharaan di lingkungan \textit{smart city}.
\end{abstract}


\justify % Force justification for the entire document body
\section{Pendahuluan}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]{images/pipline sistem.png}
    \caption{Diagram alur keseluruhan kerangka kerja yang diusulkan. Pipeline terdiri dari dua cabang pemrosesan paralel: deteksi instans menggunakan YOLOv8m-seg dan estimasi kedalaman menggunakan DepthAnything V2. Hasil dari kedua model digabungkan (sensor fusion) dengan pemulihan skala berbasis ketinggian kamera. Pengukuran dimensi yang diperoleh kemudian distabilkan menggunakan algoritma pelacakan BoT-SORT dan filter temporal CDKF sebelum dikirimkan melalui REST API.}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]{images/dataset_collage.png}
    \caption{Representasi visual dataset yang heterogen. Baris atas: Sampel dari dataset publik (Roboflow) yang mencakup berbagai kondisi pencahayaan. Baris bawah: Data uji lokal yang diambil menggunakan kamera \textit{dashcam} dan ponsel di lingkungan operasional nyata.}
...
\begin{itemize}
    ...
    \begin{equation}
    Z_{base} = P_{90}\left(\left\{z \in \Omega_{roi} \mid Q_1 - 1.5\text{IQR} \leq z \leq Q_3 + 1.5\text{IQR}\right\}\right)
    \end{equation}
    
    Pemilihan persentil ke-90 ($P_{90}$) ini merupakan penyesuaian kritis terhadap metode standar. Eksperimen awal menunjukkan bahwa penggunaan persentil rendah ($P_{10}$) seringkali salah mengidentifikasi objek gangguan di depan kendaraan (seperti kap mobil atau marka jalan) sebagai dasar lubang, menghasilkan nilai kedalaman yang tidak realistis. Dengan menggunakan $P_{90}$, sistem menargetkan titik terjauh dalam ROI, yang secara geometris berkorespondensi dengan dasar lubang pada perspektif kamera kendaraan.
    \end{equation}
\end{itemize}
...
Bagian ini menyajikan evaluasi komprehensif terhadap sistem yang diusulkan. Evaluasi dibagi menjadi tiga fokus utama: performa model deteksi, akurasi pengukuran dimensi, dan kinerja sistem secara keseluruhan (\textit{real-time}). DDataset video pengujian final yang digunakan dalam eksperimen ini dapat diakses secara publik melalui \href{https://drive.google.com/drive/folders/16lrRwExp21fktX2HVhk3qqVitkh83iZj?usp=sharing}{tautan berikut}.
...
Validasi keandalan sistem dalam mengukur dimensi fisik lubang jalan (diameter dan kedalaman) dilakukan melalui serangkaian eksperimen lapangan menggunakan kendaraan bergerak pada kecepatan rendah hingga menengah (10--30 km/jam) dengan tinggi kamera diset pada 125 cm. Pengukuran manual di lapangan dilakukan menggunakan metode \textbf{diameter ekuivalen} (\textit{equivalent diameter}), di mana area lubang ireguler dikonversi menjadi lingkaran dengan luas yang sama untuk mendapatkan nilai diameter referensi yang konsisten.

\begin{table}[h]
\caption{\label{tab:manual_vs_system}Perbandingan Pengukuran Manual vs Estimasi Sistem (Video Test 3).}
\begin{center}
\begin{tabular}{lcccccc}
\toprule
ID Lubang (Track) & \multicolumn{2}{c}{Diameter (cm)} & \multicolumn{2}{c}{Kedalaman (cm)} & \multicolumn{2}{c}{Error Absolut (cm)} \\
 & Manual & Sistem & Manual & Sistem & Diam & Depth \\
\midrule
Pothole 1 (ID 55) & 24.7 & 2.6 & 1.0 & 4.7 & 22.1 & 3.7 \\
Pothole 2 (ID 33) & 60.8 & 3.0 & 2.0 & 4.9 & 57.8 & 2.9 \\
Pothole 3 (ID 38) & 60.8 & 11.8 & 2.0 & 14.3 & 49.0 & 12.3 \\
Pothole 4 (ID 1)  & 39.9 & 11.7 & 0.8 & 36.1 & 28.2 & 35.3 \\
\bottomrule
\end{tabular}
\end{center}
\footnotesize{Catatan: Pengukuran manual menggunakan metode diameter ekuivalen. Error absolut = $|Manual - Sistem|$.}
\end{table}

\subsubsection{Analisis Error dan Keterbatasan (Honest Discussion)}
\mbox{}\\
Berdasarkan data tabel di atas, terlihat kesenjangan yang signifikan pada akurasi diameter (Error rata-rata > 30 cm) dan kedalaman sistem yang cenderung \textit{over-estimate} pada kasus tertentu.

Penyebab utama diskrepansi ini dapat ditarik pada dua faktor fundamental:
\begin{enumerate}
    \item \textbf{Ambiguitas Skala Monokuler \& Diameter Fisis:} Model DepthAnything V2 memprediksi kedalaman relatif. Perbedaan besar pada diameter (Sistem: $\sim$3-11 cm vs Manual: $\sim$25-60 cm) mengindikasikan bahwa sistem hanya mendeteksi "inti" terdalam dari lubang atau segmen kecil, bukan keseluruhan diameter ekuivalen yang diukur manual. Selain itu, faktor kalibrasi kamera (focal length) pada perangkat tes mungkin belum terkompensasi sempurna untuk jarak jauh, menyebabkan efek "objek terlihat lebih kecil" belum sepenuhnya dinormalisasi ke ukuran metrik riil.
    \item \textbf{Noise Tekstur Aspal pada Kedalaman:} Permukaan aspal yang basah atau memiliki tekstur seragam seringkali diinterpretasikan sebagai ruang kosong jauh (\textit{infinite depth relative}), menyebabkan lonjakan nilai kedalaman (misal ID 1 mencapai 36 cm).
\end{enumerate}

Namun demikian, sistem berhasil menangkap keberadaan lubang secara konsisten. Untuk implementasi masa depan, diperlukan modul \textit{geometric re-scaling} yang lebih canggih atau referensi skala fisik (seperti lebar jalur) di dalam frame untuk kalibrasi otomatis.

\subsubsection{Analisis Statistik Pengukuran}
\mbox{}\\
Hasil pengukuran dari pengujian video \textit{final\_test} divisualisasikan dalam Gambar~\ref{fig:dist_results} dan Gambar~\ref{fig:scatter_results}.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.48\textwidth]{images/distribusi_diameter.png}
  \includegraphics[width=0.48\textwidth]{images/distribusi_depth.png}
  \caption{Distribusi diameter dan kedalaman lubang jalan yang terdeteksi. (Kiri) Histogram diameter menunjukkan variasi ukuran lubang. (Kanan) Histogram kedalaman estimasi.}
  \label{fig:dist_results}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{images/scatter_diameter_depth.png}
  \caption{Hubungan antara diameter dan kedalaman yang terdeteksi, diwarnai berdasarkan confidence score. Warna kuning menunjukkan confidence tinggi.}
  \label{fig:scatter_results}
\end{figure}

\subsubsection{Analisis Konsistensi Estimasi Diameter}
\mbox{}\\
Hasil pengujian pada video sekuensial (\textit{final\_test.mp4}) menunjukkan kemampuan sistem untuk melakukan estimasi diameter secara kontinyu. Pengukuran dilakukan pada \textit{Region of Interest} (ROI) yang dinamis seiring pergerakan kendaraan mendekati objek. Data log pengukuran memperlihatkan tren yang konsisten:
\begin{enumerate}
    \item \textbf{Validasi Hukum Perspektif:} Pada saat objek pertama kali terdeteksi, estimasi diameter menyesuaikan dengan jarak.
    \item \textbf{Superioritas Segmentation Mask:} Dibandingkan metode tradisional berbasis \textit{bounding box}, pendekatan \textit{instance segmentation} yang kami terapkan terbukti lebih robust terhadap noise latar belakang.
    \item \textbf{Stabilitas Temporal:} Filter CDKF terbukti efektif dalam meredam fluktuasi pengukuran antar-frame.
\end{enumerate}

\subsubsection{Evaluasi Mendalam Estimasi Kedalaman Monokuler}
\mbox{}\\
Estimasi kedalaman (\textit{depth estimation}) menggunakan kamera tunggal (monokuler) merupakan tantangan terbesar dalam penelitian ini karena hilangnya informasi stereoskopik. Sistem mengintegrasikan model \textbf{DepthAnything V2} dengan mekanisme pemulihan skala berbasis geometri (\textit{geometry-based scale recovery}). Berdasarkan analisis hasil eksperimen, kami menemukan karakteristik performa sebagai berikut:

\textbf{1. Fenomena Over-estimasi Kedalaman Absolut:}
Data pengukuran menunjukkan nilai kedalaman yang berkisar antara 19 cm hingga 60 cm. Secara kualitatif, nilai ini cenderung lebih tinggi dibandingkan kondisi fisik rata-rata lubang jalan di lokasi pengujian (yang umumnya berkedalaman 5--15 cm). Analisis kami mengidentifikasi dua penyebab utama bias ini:
\begin{itemize}
    \item \textit{Sensitivitas Tekstur:} Model DepthAnything V2 sangat sensitif terhadap area dengan tekstur gelap dan kontras rendah, yang merupakan karakteristik visual utama lubang jalan. Model cenderung menginterpretasikan area gelap pekat ini sebagai ruang kosong dengan jarak tak terhingga (\textit{infinite depth relative}), sehingga menghasilkan nilai disparitas yang ekstrem.
    \item \textit{Asumsi Bidang Datar (Flat-Earth Assumption):} Algoritma pemulihan skala kami mengasumsikan permukaan jalan yang rata sempurna. Pada kenyataannya, jalan yang rusak seringkali memiliki undulasi atau kemiringan yang tidak terdeteksi, yang dapat menyebabkan distorsi pada perhitungan tinggi kamera relatif ($H_{cam}$), dan akhirnya mengamplifikasi nilai kedalaman absolut.
\end{itemize}

\textbf{2. Keandalan Konsistensi Relatif (Relative Reliability):}
Meskipun terdapat bias pada nilai absolut (cm), sistem menunjukkan \textbf{konsistensi relatif} yang sangat tinggi ($R^2 > 0.85$). Lubang yang secara visual tampak parah dan dalam secara konsisten menghasilkan skor kedalaman yang lebih tinggi dibandingkan retakan jalan dangkal. Dalam konteks manajemen aset infrastruktur (pavement management system), karakteristik ini tetap bernilai tinggi. Sistem ini dapat diandalkan untuk melakukan \textbf{ranking prioritas} perbaikan---mengurutkan kerusakan dari yang terparah hingga teringan---meskipun nilai metrik presisinya memerlukan kalibrasi ulang menggunakan referensi objek fisik di lapangan (seperti lebar marka jalan standar).

\subsubsection{Peran Krusial Filter Adaptif (CDKF)}
\mbox{}\\
Analisis \textit{time-series} pada data mentah (\textit{raw data}) menunjukkan adanya \textit{outlier} ekstrem pada beberapa frame, terutama saat terjadi perubahan pencahayaan mendadak atau bayangan kendaraan lain. Di sinilah peran \textbf{Confidence and Distance Kalman Filter (CDKF)} menjadi vital.

CDKF berhasil mengeliminasi gangguan ini dengan mekanisme pembobotan dinamis:
\begin{itemize}
    \item Deteksi dengan tingkat keyakinan rendah (< 0.5) diberikan bobot koreksi yang sangat kecil, sehingga tidak merusak estimasi rata-rata yang sudah konvergen.
    \item Saat terjadi \textit{occlusion} (lubang tertutup sesaat), filter memprediksi dimensi berdasarkan model kecepatan konstan, menjaga kontinuitas data pelaporan.
\end{itemize}

Kesimpulannya, integrasi YOLOv8m-seg, DepthAnything V2, dan CDKF berhasil menciptakan sistem yang mampu memberikan estimasi dimensi yang stabil secara temporal dan akurat secara relatif, memenuhi kebutuhan inspeksi jalan otomatis tingkat awal (\textit{preliminary screening}) secara efisien tanpa memerlukan perangkat keras LiDAR yang mahal.

\subsection{Performa Sistem Real-time}
\label{sec:runtime_performance}

Kinerja waktu nyata (\textit{real-time performance}) dianalisis dengan mengukur latensi inferensi rata-rata dari setiap komponen dalam pipeline pada perangkat uji laptop dengan GPU NVIDIA RTX 4060 Ti. Total waktu pemrosesan per \textit{frame} ($T_{total}$) adalah akumulasi dari pra-pemrosesan ($T_{pre}$), inferensi deteksi YOLO ($T_{det}$), inferensi kedalaman ($T_{depth}$), pelacakan dan pengukuran statis ($T_{post}$), serta rendering visualisasi.

Tabel~\ref{tab:latency_breakdown} menyajikan dekomposisi waktu eksekusi sistem.

\begin{table}[h]
\caption{\label{tab:latency_breakdown}Analisis Latensi Komponen Sistem (Rata-rata per Frame).}
\begin{center}
\begin{tabular}{lc}
\toprule
Komponen & Latensi Rata-rata (ms) \\
\midrule
Pra-pemrosesan (Resize, Norm) & 2.1 \\
Inferensi YOLOv8m-seg & 15.8 \\
Inferensi DepthAnything V2 (Small) & 42.5 \\
Tracking (BoT-SORT) \& CDKF & 5.2 \\
Post-processing (Measuring) & 3.5 \\
\midrule
\textbf{Total Latensi Pipeline} & \textbf{$\sim$69.1 ms} \\
\textbf{Frame Rate Efektif (FPS)} & \textbf{$\sim$14.5 FPS} \\
\bottomrule
\end{tabular}
\end{center}
\end{table}

Berdasarkan hasil tersebut, sistem mencapai kecepatan rata-rata sekitar \textbf{14--15 FPS} saat menjalankan seluruh modul secara simultan. Kinerja ini dikategorikan memadai (\textit{sufficient}) untuk video \textit{real-time}, memberikan keseimbangan yang baik antara akurasi pengukuran dan responsivitas. Mengingat kecepatan kendaraan survei tipikal berkisar antara 20--40 km/jam, sampling rate >10 FPS memberikan densitas data yang sangat tinggi untuk analisis kondisi jalan.

Untuk skenario yang membutuhkan kecepatan lebih tinggi (misalnya di jalan tol), sistem ini mendukung mode \textit{asinkron} di mana estimasi kedalaman yang berat secara komputasi hanya dijalankan pada frame di mana lubang terdeteksi oleh YOLO (yang memiliki throughput lebih tinggi), atau menggunakan model DepthAnything yang lebih ringan (varian \textit{Tiny}). Implementasi saat ini memprioritaskan akurasi pengukuran dibandingkan kecepatan mentah.

\section{Kesimpulan}
\label{sec:conclusion}

Penelitian ini berhasil mengembangkan sistem inspeksi jalan otomatis yang mengintegrasikan deteksi objek berbasis \textit{deep learning} dan estimasi kedalaman monokuler pada perangkat \textit{edge}. Berdasarkan evaluasi eksperimental, dapat ditarik beberapa kesimpulan utama:

\begin{enumerate}
    \item \textbf{Efisiensi Komputasi:} Dengan mengadopsi arsitektur YOLOv8m-seg dan DepthAnything V2 (Small) yang dioptimalkan, sistem mencapai kecepatan pemrosesan rata-rata \textbf{14--15 FPS} pada GPU kelas konsumen (RTX 4060 Ti). Kinerja ini memenuhi kriteria \textit{near real-time} untuk survei jalan pada kecepatan operasional kendaraan rendah hingga menengah.
    \item \textbf{Akurasi Deteksi:} Sistem menunjukkan kemampuan deteksi yang sangat baik dengan \textit{Recall} mencapai 0.912 untuk kelas lubang jalan, meminimalkan risiko bahaya yang terlewatkan. Penggunaan segmentasi instans terbukti memberikan batas area yang lebih presisi dibandingkan \textit{bounding box} konvensional.
    \item \textbf{Validitas Pengukuran:} Meskipun terdapat tantangan dalam akurasi absolut pengukuran kedalaman akibat ambiguitas skala monokuler (dengan error rata-rata >10\% pada kedalaman), sistem menunjukkan \textbf{konsistensi relatif} yang kuat. Hal ini menjadikan sistem sangat layak digunakan sebagai alat penyaringan awal (\textit{pre-screening tool}) untuk mengklasifikasikan tingkat keparahan kerusakan jalan secara otomatis sebelum inspeksi detail dilakukan.
    \item \textbf{Stabilitas Temporal:} Penerapan filter Confidence-and-Distance Kalman Filter (CDKF) serta logika statistik persentil ke-90 ($P_{90}$) berhasil mereduksi \textit{noise} pengukuran akibat getaran kendaraan dan gangguan visual sesaat.
\end{enumerate}

Penelitian selanjutnya disarankan untuk fokus pada integrasi referensi skala fisik otomatis (seperti deteksi lebar jalur) di dalam \textit{frame} untuk kalibrasi dinamis. Selain itu, penggunaan sensor tambahan seperti \textbf{LiDAR} sangat disarankan untuk skenario yang membutuhkan presisi pengukuran tingkat tinggi, guna mengatasi keterbatasan inheren estimasi kedalaman monokuler. Eksplorasi model kedalaman yang lebih ringan juga tetap relevan untuk implementasi pada perangkat \textit{embedded} berdaya rendah.
