...
\begin{document}
\history{Received December 1, 2025, accepted December 10, 2025, date of publication December 15, 2025, date of current version December 20, 2025.}
\doi{10.1109/ACCESS.2025.DOI}

\title{Integrated Pothole Detection and Measurement System using YOLOv8 and DepthAnything V2 with Temporal Filtering}
\author{\uppercase{Pangeran Juhrifar Jafar}\authorrefmark{1},
\IEEEmembership{Student Member, IEEE}}
\address[1]{Hasanuddin University, Makassar, Indonesia (e-mail: pangeranjuhrifar@gmail.com)}
...
\corresp{Corresponding author: Pangeran Juhrifar Jafar (e-mail: pangeranjuhrifar@gmail.com).}

\begin{abstract}
Road infrastructure maintenance is critical for transportation safety and economic efficiency, yet traditional manual inspection methods remain reactive, subjective, and hazardous. This paper presents a comprehensive "Smart Information System" for the real-time detection, accurate measurement, and automatic reporting of potholes. We propose an integrated framework utilizing the lightweight \textbf{YOLOv8n} model for high-speed pothole detection and \textbf{DepthAnything V2} for fine-grained monocular depth estimation. To address the challenge of scale ambiguity in monocular vision, we implement a robust height-based scale recovery method. Furthermore, we introduce a novel \textbf{Confidence and Distance Kalman Filter (CDKF)} combined with \textbf{BoT-SORT} tracking to temporally smooth measurements and reject outliers using robust statistics (IQR and median filtering). The system integrates a REST API for automatic data transmission to a centralized dashboard, enabling proactive infrastructure management. Experimental results demonstrate that our pipeline achieves a balance between real-time processing speed and measurement accuracy, offering a viable solution for intelligent transportation systems in resource-constrained environments.
\end{abstract}

\begin{keywords}
Pothole Detection, Monocular Depth Estimation, YOLOv8n, DepthAnything V2, Kalman Filter, Smart City, Automated Road Inspection.
\end{keywords}
...
\PARstart{R}{oad} infrastructure serves as the fundamental backbone for a nation's economic and social development. High-quality roads facilitate mobility, support economic growth, and ensure public service accessibility. However, maintaining this infrastructure presents a significant challenge, particularly in countries with complex geographies like Indonesia. According to the Ministry of Public Works and Housing, approximately 30\% of Indonesia's 496,000 km road network suffers from damage ranging from mild to severe \cite{kemenpupr}. Among these issues, potholes are the most prevalent phenomenon, exacerbated by high rainfall and heavy traffic loads. Potholes not only compromise driving comfort but also pose serious safety risks and cause significant economic losses through vehicle damage and accidents.

Traditionally, road damage detection has relied on manual inspection methods and public reporting. These approaches are inherently reactive, labor-intensive, hazardous for inspectors, and often prone to subjective error. There is a significant gap between the need for proactive maintenance and the capabilities of manual detection systems. Although some smart city initiatives have been initiated in major cities, a comprehensive, automated system that effectively integrates detection, accurate measurement, and reporting is still lacking.

The advent of Computer Vision and Deep Learning offers a transformative solution to these challenges. Convolutional Neural Networks (CNNs) have revolutionized automated defect detection. Recent studies have demonstrated the efficacy of models like YOLO (You Only Look Once) for real-time pothole detection \cite{gorro2024}. However, mere detection is insufficient for effective maintenance planning; knowing the size and depth of a pothole is crucial for prioritizing repairs and estimating material costs. Most existing vision-based solutions rely on 2D object detection, failing to provide the 3D depth information required for volumetric assessment.

While stereo vision and LiDAR can provide depth information, they are often cost-prohibitive and computationally expensive for wide-scale deployment. Monocular depth estimation has emerged as a cost-effective alternative, but it faces the challenge of "scale ambiguity"—outputting relative depth maps that lack absolute metric units. Recent advancements, such as Wang et al. (2025) \cite{wang2025}, have begun to address these limitations by integrating temporal coherence, yet seamless integration into a real-time reporting workflow remains an open research area.

This paper proposes an integrated "Smart Information System" that addresses these limitations by combining state-of-the-art deep learning models with robust engineering pipelines. Our contributions are as follows:
\begin{enumerate}
    \item Implementation of a real-time detection system using the lightweight \textbf{YOLOv8n} model, optimized for edge deployment.
    \item Development of a robust measurement module utilizing \textbf{DepthAnything V2} for fine-grained monocular depth estimation, coupled with a height-based scale recovery technique to provide absolute metric measurements (diameter and depth).
    \item Integration of a \textbf{Confidence and Distance Kalman Filter (CDKF)} and \textbf{BoT-SORT} tracking to ensure temporal consistency and smoothing of measurements across video frames.
    \item Creation of an automated reporting pipeline that transmits geolocated defect data via REST API to a centralized dashboard, facilitating immediate action by road authorities.
\end{enumerate}

The remainder of this paper is organized as follows: Section II discusses related work. Section III details our proposed methodology, including the system architecture and algorithms. Section IV presents the experimental results and analysis. Finally, Section V concludes the paper and discusses future work.
...
\bibitem{kemenpupr}
Kementerian Pekerjaan Umum dan Perumahan Rakyat (PUPR), ``Statistik Jalan Indonesia,'' 2024. [Online]. Available: https://pu.go.id
...
is a student at Hasanuddin University, majoring in Information Systems. His research interests include Computer Vision, Deep Learning, and Intelligent Transportation Systems.
\end{IEEEbiography}
